...
|---|---|---|---|
| Systems / product reasoning | **4** | 2026-08: this reasoning now generalizes past the plant model into infra/deployment architecture — independently proposed decoupling the AI-chat serverless function from the Firebase migration ("would it be easy to do this before the data migration?") and correctly reasoned out why (the web app already has plant context client-side; no DB needed for v1). That's real sequencing/dependency judgment, not just recall. Also demanded a validation gate before building the fitted-curve UI, and pushed back correctly on both a gate bug (β ceiling firing on 37% of plants) and a UI call ("wouldn't the line look ugly on top of the dots?" — right). Earlier (2026-07-19): independently described a filtering/state-space approach for the moisture curve. | Keep applying it to new problem types. |
| Software / product vocabulary | **3 ↑** | 2026-08: fluently generated her own infra proposal ("move data to Firebase and add the AI key to Vercel secrets") using terms like *serverless function*, *environment variable*, *backend* correctly and unprompted — not just recognizing them when I used them. Still knows `idempotent`, PRs, branches, localStorage. | Bank a few architecture terms (component, state, prop, selector); next is recognizing infra vocab (serverless, env var, edge function) without a reminder across sessions. |

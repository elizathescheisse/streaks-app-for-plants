...
- **Capture clear ones** as a one-line entry in the HUMAN.md motivation log (what was happening → effect). Only log genuine signals, not every mood. **These are living files, not a running diary — updating them constantly is just noise. Touch them only when something genuinely changed; when in doubt, don't.**
